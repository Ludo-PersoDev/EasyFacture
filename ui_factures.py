  ...
  with ui.card().classes(
    ...
    def rafraichir_liste():
      ...
      for r in rows:
        ...
        if item["statut"] not in ["Payée", "Annulée"] and item.get(
          ...
          try:
            date_ech = datetime.strptime(
                str(item["date_echeance"]).strip(), "%Y-%m-%d"
            ).date()
        ...
        if item.get("statut") == "Payée" and date_paiement_fr:
          if mode_regl:
            item["statut_affichage"] = (
                f"Payée le {date_paiement_fr} par {mode_regl}"
            )
          else:
      ...
      columns = [
          ...
          },
          {
              ...
              "cellClassRules": {
                  ...
                  "text-rose-600 font-medium text-xs": (
                      'x && x.startsWith("❌")'
                  ),
              },
          },
          {
              ...
              "cellClassRules": {
                  (
                      "bg-slate-100 text-slate-800 font-semibold text-xs"
                      " border-slate-300"
                  ): 'x && x.startsWith("Brouillon")',
                  (
                      "bg-blue-100 text-blue-800 font-semibold text-xs"
                      " border-blue-300"
                  ...
                  (
                      "bg-rose-100 text-rose-800 font-semibold text-xs"
                      " border-rose-300"
        ...
        def update_action_bar():
          ...
          with btn_group:
            if selected:
              ...
              has_recap_opt = selected.get("client_recap", 0) == 1
              ...
              if has_recap_opt:
                ...
                ).props("dense outline color=info font-bold text-sky-700")
              ...
              if statut != "Payée" and statut != "Annulée":
                ui.button(
                    "Marquer Payée",
                    icon="check_circle",
                    on_click=lambda: ouvrir_dialog_paiement(selected),
              ...
              if statut != "Annulée":
                ui.button(
                    ...
                    on_click=lambda: confirmer_avoir_facture(selected),
                ).props("dense flat color=warning font-bold").tooltip(
                    "Annule la facture en émettant un avoir légal"
                )
            else:
              ui.label(
                  "💡 Cliquez sur une facture pour l'imprimer, voir son"
                  " récapitulatif, l'envoyer ou enregistrer son paiement."
    ...
    with top_bar:
      with ui.row().classes("items-center gap-4 flex-wrap flex-1"):
        ui.label("Factures").classes("text-lg font-semibold text-slate-700")
    ...
    def ouvrir_dialog_recap(facture):
      with ui.dialog() as dialog, ui.card().classes(
        ...
        ).classes("text-xl font-bold text-slate-800 border-b pb-2 w-full")

        with ui.row().classes("w-full items-center gap-4"):
          ui.label(
              f"Client : {facture.get('client_nom', '')} | Période de référence"
              " du mois"
        ...
        if not interventions:
          ...
          for it in interventions:
            ...
            sem_key = (
                f"Semaine du {debut_sem.strftime('%d %B').lower()} au"
                f" {fin_sem.strftime('%d %B').lower()}"
            )
          ...
          def rafraichir_tableau_recap():
            ...
            for etab in etabs_list:
              table_html.append(
                  f"<th class='border border-slate-300 p-2.5"
                  f" text-center'>{etab}</th>"
              )
            table_html.append(
                "<th class='border border-slate-300 p-2.5 text-center"
                " bg-slate-200'>Total semaine</th>"
            )
            ...
            for sem_lib, items_sem in semaines_dict.items():
              table_html.append("<tr>")
              table_html.append(
                  f"<td class='border border-slate-300 p-2.5 font-semibold"
                  f" bg-slate-50'>{sem_lib}</td>"
              )
              ...
              for etab in etabs_list:
                matches = [
                    x
                    for x in items_sem
                    if (x["etablissement_nom"] or "Autre") == etab
                ]
                cell_texts = []
                for m in matches:
                  d_fr = formater_date_fr(m["date"])
                  montant_ligne = (m["prix_final_ht"] or 0.0) * (
                      m["quantite"] or 1.0
                  )
                  total_semaine += montant_ligne
                  nombre_semaine += 1

                  desc_courte = m["prest_nom"] or f"Intervention N°{m['id']}"
                ...
                contenu_cellule = (
                    "<br>".join(cell_texts) if cell_texts else "-"
                )
                table_html.append(
                    f"<td class='border border-slate-300 p-2.5 text-center"
                    f" text-xs'>{contenu_cellule}</td>"
                )
              ...
              table_html.append(
                  f"<td class='border border-slate-300 p-2.5 text-center"
                  f" font-bold bg-slate-50'>{total_semaine:.0f}€<br/><span"
                  " class='text-xs font-normal"
                  f" text-slate-500'>({nombre_semaine}"
                  f" {intitule_choisi})</span></td>"
              )
              table_html.append("</tr>")

            table_html.append("<tr class='bg-primary text-white font-bold'>")
            table_html.append(
            ...
            table_html.append(
                f"<td class='border border-slate-300 p-3"
                f" text-center'>{total_general_mois:.0f}€<br/><span"
                " class='text-xs font-normal"
                f" text-sky-200'>({total_general_nombre}"
                f" {intitule_choisi})</span></td>"
            )
            table_html.append("</tr>")
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-4"):
          ...
          ui.button(
              ...
              icon="print",
              on_click=lambda: imprimer_recap_pdf(
                  facture, intitule_select.value
              ),
          ).props("color=primary font-bold")
      ...
      with ui.dialog() as dialog, ui.card().classes(
        ...
        with ui.row().classes("w-full gap-8 items-start"):
          ...
          with ui.column().classes("w-1/2 space-y-5"):
            ui.label("1. Paramètres de la facture").classes(
            ...
            def calculer_date_echeance():
              ...
              dt_echeance_obj = date_aujourdhui_obj + timedelta(days=jours)
          ...
          with ui.column().classes("w-1/2 space-y-4 flex-1"):
            ui.label("2. Sélection des prestations à facturer").classes(
                "text-sm font-bold text-slate-700 uppercase tracking-wide"
            )
            ...
            with ui.row().classes("w-full gap-2 items-center flex-wrap"):
              ...
              btn_m_cours = ui.button("Mois en cours", icon="today").props(
                  "dense outline color=primary text-xs"
              )
              btn_deselect = ui.button("Tout décocher", icon="clear_all").props(
                  "dense flat color=negative text-xs"
              )
              btn_select_all = ui.button("Tout cocher", icon="select_all").props(
                  "dense flat color=slate text-xs"
              )

            prestations_container = ui.column().classes(
                "w-full h-80 overflow-y-auto border border-slate-200 p-4"
                " rounded-xl bg-slate-50/50 space-y-2.5"
            )
            totaux_container = ui.row().classes(
                "w-full justify-between items-center bg-slate-100 p-4"
                " rounded-xl font-semibold text-slate-800"
            )
        ...
        def verifier_et_afficher_options_recap(client_id):
          ...
          if recap_active:
            with recap_options_container:
              with ui.card().classes(
                  "w-full p-4 bg-sky-50/60 border border-sky-200 rounded-xl"
                  " space-y-2 shadow-sm"
              ):
                ui.label(
                    "📊 Options du récapitulatif des prestations"
                ).classes(
                    "text-xs font-bold text-sky-800 uppercase tracking-wide"
                )
        ...
        def cocher_par_periode(periode_mode):
          ...
          if periode_mode == "mois_precedent":
            ...
            dernier_jour_m_prec = premier_jour_ce_mois - timedelta(days=1)
        ...
        btn_m_prec.on_click(lambda: cocher_par_periode("mois_precedent"))
        ...
        def maj_totaux():
          ...
          for item_id, item_info in cochables_holder.items():
            if item_info["checkbox"].value:
              d = item_info["data"]
              ht = (d["prix_final_ht"] or 0.0) * (d["quantite"] or 1.0)
        ...
        with ui.row().classes(
            "w-full justify-end gap-3 pt-4 border-t border-slate-100"
        ):
          ui.button("Annuler", on_click=dialog.close).props("flat color=slate")
          ui.button(
              "Valider et Générer", icon="receipt", on_click=generer_facture
    ...
    def ouvrir_dialog_paiement(facture):
      with ui.dialog() as dialog, ui.card().classes(
        ...
        ui.label(
            f"Enregistrer le paiement - Facture {facture['numero_facture']}"
        ...
        mode_select = ui.select(
            options=options_modes, value="Virement", label="Mode de règlement"
        ).classes("w-full").props("dense outlined")

        ui.label("Date du règlement :").classes(
            "text-sm text-slate-600 font-medium"
        )
        date_selectionnee = ui.date(value=str(datetime.now().date())).classes(
            "w-full"
        )
        ...
        def confirmer_paiement():
          ...
          ui.notify(
              ...
              f" {formater_date_fr(date_paiement)} par {mode_paiement} !",
              type="positive",
              icon="check_circle",
          )
          dialog.close()
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-4"):
          ui.button("Annuler", on_click=dialog.close).props("flat color=slate")
          ui.button(
              "Confirmer le paiement", icon="check", on_click=confirmer_paiement
          ).props("color=positive font-bold")

      dialog.open()

    def confirmer_avoir_facture(facture):
      with ui.dialog() as dialog, ui.card().classes("p-6 space-y-4 max-w-md"):
        ...
        ui.label(
            ...
            " facturation ultérieure et la facture sera classée 'Annulée'."
        ...
        def annuler_et_creer_avoir():
          ...
          dialog.close()
          ui.notify(
              f"Facture {facture['numero_facture']} annulée. Les prestations"
              " ont été réouvertes.",
              type="info",
              icon="assignment_return",
          )
    ...
    def imprimer_facture_pdf(facture):
      try:
        ...
        ui.notify(
            f"Erreur lors de la génération PDF : {str(e)}", type="negative"
        )
    ...
    def imprimer_recap_pdf(facture, intitule="Intervention(s)"):
      try:
        ...
        ui.notify(
            "PDF du récapitulatif ouvert avec succès !",
            type="positive",
            icon="print",
        )
      except Exception as e:
        ui.notify(
            f"Erreur lors de la génération du récap : {str(e)}", type="negative"
        )
    ...
    def ouvrir_dialog_email(facture):
      ...
      with ui.dialog() as dialog, ui.card().classes(
        ...
        ui.label(f"Envoyer la Facture {facture['numero_facture']}").classes(
            "text-lg font-bold text-slate-800 border-b pb-2 w-full"
        )
        ui.label(
        ...
        email_input = ui.input("Destinataire (Email)", value=email_client).classes(
            "w-full"
        )
        objet_input = ui.input(
            "Objet du message", value=f"Facture {facture['numero_facture']}"
        ).classes("w-full")
        ...
        def confirmer_envoi():
          ...
          if recap_actif:
            try:
              recap_path = pdf_factures.generer_pdf_recap_facture(
                  facture["id"]
              )
            except Exception as e:
              print(
                  "Erreur lors de la génération du récapitulatif :"
                  f" {e}"
              )
