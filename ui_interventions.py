...
def calculer_statut_prestation(date_interv_str, facture_id=None, statut_facture=None):
    ...
    try:
        date_obj = datetime.strptime(str(date_interv_str).strip(), "%Y-%m-%d").date()
...
def render_interventions():
    ui.label("Suivi des Prestations Réalisées").classes("text-2xl font-bold text-slate-800 mb-6")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-slate-200 rounded-xl space-y-4"):
        ...
        columns = [
            {'headerName': 'N° Intervention', 'field': 'numero_intervention', 'align': 'left', 'sortable': True, 'filter': 'agTextColumnFilter'},
            {'headerName': 'Date', 'field': 'date_fr', 'align': 'center', 'sortable': True, 'filter': 'agDateColumnFilter'},
            {'headerName': 'Client', 'field': 'client_nom', 'align': 'left', 'sortable': True, 'filter': 'agTextColumnFilter'},
            {'headerName': 'Site', 'field': 'site_txt', 'align': 'left', 'sortable': True, 'filter': 'agTextColumnFilter'},
            {'headerName': 'Prestation', 'field': 'nom_prestation_txt', 'align': 'left', 'sortable': True, 'filter': 'agTextColumnFilter'},
            {'headerName': 'Durée (h)', 'field': 'quantite', 'align': 'center', 'sortable': True},
            {'headerName': 'Horaires', 'field': 'heure_txt', 'align': 'center', 'sortable': True},
            {'headerName': 'Montant HT', 'field': 'prix_txt', 'align': 'right', 'sortable': True},
            {'headerName': 'Commentaire', 'field': 'commentaire_txt', 'align': 'left', 'sortable': True, 'filter': 'agTextColumnFilter'},
            {
                'headerName': 'Statut', 
                ...
                'cellClassRules': {
                    'bg-blue-100 text-blue-800 font-bold border-blue-300': 'x === "Planifiée"',
                    'bg-amber-100 text-amber-800 font-bold border-amber-300': 'x === "Réalisée"',
                    'bg-orange-100 text-orange-800 font-bold border-orange-300': 'x === "Facturée"',
                    'bg-emerald-100 text-emerald-800 font-bold border-emerald-300': 'x === "Payée"',
                }
            }
        ]
        ...
        with ui.row().classes("w-full justify-between items-center pt-3 border-t border-slate-200 min-h-[48px]"):
            action_label = ui.label("💡 Cliquez sur une ligne du tableau pour l'éditer ou la supprimer.").classes("text-xs text-slate-400 font-medium italic py-1")
        ...
        def update_action_bar():
            ...
            if selected:
                ...
                action_label.set_text("💡 Cliquez sur une ligne du tableau pour l'éditer ou la supprimer.")
        ...
        def charger_donnees():
            ...
            for r in rows:
                ...
                item['nom_prestation_txt'] = item['prestation_nom'] or "Prestation libre"
                item['site_txt'] = item['etablissement_nom'] or "-"
                item['heure_txt'] = f"{item['heure_debut'] or ''} - {item['heure_fin'] or ''}".strip(" -") or "-"
                item['prix_txt'] = f"{(item['prix_final_ht'] or 0.0) * (item['quantite'] or 1.0):.2f} € HT"
        ...
        with top_bar:
            with ui.row().classes("items-center gap-4 flex-wrap flex-1"):
                ...
                checkbox_payees = ui.checkbox("Masquer les payées", value=False).classes("text-slate-700 font-medium")
            ...
            ui.button("Nouvelle Prestation", icon="add", on_click=lambda: ouvrir_dialog_intervention()).props("color=primary font-bold")
        ...
        def tenter_edition():
            ...
            if statut in ["Facturée", "Payée"]:
                ui.notify(f"Impossible de modifier la prestation « {interv['numero_intervention']} » : elle est déjà {statut.lower()} !", type="warning", icon="lock")
                return

            if statut == "Réalisée":
                ui.notify(f"Attention : la prestation « {interv['numero_intervention']} » est déjà indiquée comme RÉALISÉE.", type="warning", icon="warning")
        ...
        def tenter_suppression():
            ...
            statut = interv.get('statut_dynamique', '')
            if statut in ["Facturée", "Payée"]:
                ui.notify(f"Impossible de supprimer la prestation « {interv['numero_intervention']} » : elle est déjà {statut.lower()} !", type="warning", icon="lock")
                return

            confirmer_suppression(interv['id'], interv['numero_intervention'], statut_realisee=(statut == "Réalisée"))

        # DIALOGUE DE CRÉATION / ÉDITION
        def ouvrir_dialog_intervention(interv_existante=None):
            is_edition = interv_existante is not None
            titre = f"Éditer Prestation « {interv_existante['numero_intervention']} »" if is_edition else "Nouvelle Prestation Directe"
            ...
            with dialog, ui.card().classes("p-6 space-y-4 w-full max-w-lg"):
                ui.label(titre).classes("text-xl font-bold text-slate-800 border-b pb-2 w-full")

                if is_edition and interv_existante.get('statut_dynamique') == "Réalisée":
                    ui.label("⚠️ Cette prestation est marquée comme RÉALISÉE. Toute modification impactera la facture à venir.").classes("p-2 bg-amber-50 text-amber-800 border border-amber-200 rounded text-xs w-full")

                client_options = {c['id']: c['nom_societe'] for c in clients}
                val_client_init = interv_existante['client_id'] if is_edition else clients[0]['id']
                client_select = ui.select(client_options, label="Client", value=val_client_init).classes("w-full")

                etab_select = ui.select({}, label="Site / Établissement").classes("w-full")
                prest_select = ui.select({}, label="Cours / Prestation").classes("w-full")
                    ...
                    try:
                        ...
                        if tarifs_spec:
                            for t in tarifs_spec:
                                px = t['prix_specifique_ht'] if t['prix_specifique_ht'] is not None else t['prix_ht']
                                options[t['id']] = f"{t['designation']} ({px:.2f} €/h - Tarif spécifique)"
                        else:
                            cat = conn.execute("SELECT * FROM prestations ORDER BY designation ASC").fetchall()
                            for p in cat:
                                options[p['id']] = f"{p['designation']} ({p['prix_ht']:.2f} €/h)"
                    finally:
                        conn.close()

                    prest_select.options = options
                    val_prest_init = interv_existante['prestation_id'] if (is_edition and interv_existante['prestation_id']) else (list(options.keys())[0] if options else None)
                    prest_select.value = val_prest_init

                def charger_etablissements(client_id):
                    conn = database.get_conn()
                    try:
                        etabs = conn.execute("SELECT * FROM etablissements WHERE client_id=?", (client_id,)).fetchall()
                    finally:
                        conn.close()
                    ...
                    if etabs:
                        ...
                        etab_select.value = interv_existante['etablissement_id'] if is_edition else etabs[0]['id']
                ...
                client_select.on_value_change(lambda e: (charger_etablissements(e.value), charger_prestations_client(e.value)))

                val_date_init = interv_existante['date'] if is_edition else datetime.now().strftime("%Y-%m-%d")
                date_input = ui.input("Date d'exécution", value=val_date_init).props("dense outlined").classes("w-full")
                with date_input:
                ...
                val_h_deb = interv_existante['heure_debut'] if (is_edition and interv_existante['heure_debut']) else "14:00"
                val_h_fin = interv_existante['heure_fin'] if (is_edition and interv_existante['heure_fin']) else "16:00"

                with ui.row().classes("w-full gap-4 items-center"):
                    h_debut = ui.input("Début", value=val_h_deb).props("dense outlined").classes("w-1/3")
                    with h_debut:
                        with ui.menu() as menu_h1:
                            ui.time().props("format24h minute-step=5").bind_value(h_debut)
                        with h_debut.add_slot('append'):
                            ui.icon('schedule').classes('cursor-pointer').on('click', menu_h1.open)

                    h_fin = ui.input("Fin", value=val_h_fin).props("dense outlined").classes("w-1/3")
                    with h_fin:
                        with ui.menu() as menu_h2:
                            ui.time().props("format24h minute-step=5").bind_value(h_fin)
                        with h_fin.add_slot('append'):
                            ui.icon('schedule').classes('cursor-pointer').on('click', menu_h2.open)

                    duree_label = ui.label("Durée : 2.0 h").classes("w-1/4 text-sm font-semibold text-slate-700 text-center bg-slate-100 py-2 rounded")
                ...
                remarque_input = ui.input("Remarques / Commentaire", value=interv_existante['commentaire'] if (is_edition and interv_existante['commentaire']) else "").classes("w-full")

                def sauvegarder():
                    client_id = client_select.value
                    etab_id = etab_select.value
                    prest_id = prest_select.value
                    date_val = date_input.value or datetime.now().strftime("%Y-%m-%d")
                    qte_calc = calculer_duree_heures(h_debut.value, h_fin.value)

                    if not prest_id:
                        ui.notify("Veuillez sélectionner une prestation.", type="warning")
                        return

                    conn = database.get_conn()
                    try:
                        spec = conn.execute("SELECT prix_specifique_ht FROM client_tarifs WHERE client_id=? AND prestation_id=?", (client_id, prest_id)).fetchone()
                        prest_info = conn.execute("SELECT * FROM prestations WHERE id=?", (prest_id,)).fetchone()
                        
                        prix_ht = spec['prix_specifique_ht'] if (spec and spec['prix_specifique_ht'] is not None) else (prest_info['prix_ht'] if prest_info else 0.0)
                        ...
                        if is_edition:
                            cur.execute("""
                                ...
                                WHERE id=?
                            """, (client_id, etab_id, prest_id, date_val, h_debut.value, h_fin.value, qte_calc, prix_ht, taux_tva, remarque_input.value, interv_existante['id']))
                            ...
                            cur.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'En attente', ?)
                            """, (num_interv, client_id, etab_id, prest_id, date_val, h_debut.value, h_fin.value, qte_calc, prix_ht, taux_tva, remarque_input.value))
                ...
                with ui.row().classes("w-full justify-end gap-2 mt-4"):
                    ...
                    ui.button("Enregistrer", icon="check", on_click=sauvegarder).props("color=positive font-bold")

            dialog.open()

        def confirmer_suppression(interv_id, num_interv, statut_realisee=False):
            dialog = ui.dialog()
            with dialog, ui.card().classes("p-6 space-y-4 max-w-md"):
                ...
                if statut_realisee:
                    ui.label(f"⚠️ La prestation « {num_interv} » est déjà marquée comme RÉALISÉE. Êtes-vous sûr de vouloir la supprimer ?").classes("text-amber-700 bg-amber-50 p-3 border border-amber-200 rounded text-sm font-medium")
                else:
                    ui.label(f"Voulez-vous supprimer la prestation « {num_interv} » ?").classes("text-slate-600")
