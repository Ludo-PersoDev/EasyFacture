        ...
        def ouvrir_dialogue_devis(devis_id=None):
            ...
            with dialog, ui.card().classes("w-full max-w-6xl p-6 space-y-4"):
                ...
                def sauvegarder():
                    ...
                    # --- AJOUT ICI : Génération et synchro PDF automatique à l'enregistrement ---
                    try:
                        res_cli = client_db.table("clients").select("nom_societe").eq("id", client_select.value).execute()
                        nom_soc = res_cli.data[0]['nom_societe'] if res_cli.data else "Client"
                        dossier_export = pdf_generator.obtenir_chemin_export(nom_soc, type_doc="Devis")
                        filename = f"{num_devis}.pdf"
                        pdf_path = os.path.join(dossier_export, filename)
                        pdf_generator.generer_pdf_devis(target_devis_id, pdf_path)
                        client_db.table("devis").update({"pdf_path": pdf_path}).eq("id", target_devis_id).execute()
                    except Exception as pdf_err:
                        print(f"Erreur génération PDF auto : {pdf_err}")
                    # --------------------------------------------------------------------------
