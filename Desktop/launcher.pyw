...
def executer_mise_a_jour(script_dir):
    ...
    def background_update_task():
        try:
            print("Interrogation de l'API pour récupérer l'installateur...")
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                print(f"URL de l'installateur trouvée : {exe_url}")
                
                if exe_url:
                    installer_path = os.path.join(temp_dir, "Setup_EasyFacture_Update.exe")
                    print(f"Téléchargement vers : {installer_path}")
                    urllib.request.urlretrieve(exe_url, installer_path)
                    
                    if os.path.exists(installer_path):
                        print("Téléchargement réussi. Lancement de l'installateur graphique...")
                        # On retire /VERYSILENT pour voir l'interface et comprendre ce qui se passe
                        subprocess.run([
                            installer_path, 
                            '/SUPPRESSMSGBOXES', 
                            '/NORESTART',
                        ...
                        print("Erreur : Le fichier d'installation est introuvable après téléchargement.")
                else:
                    print("Erreur : Aucun fichier .exe trouvé dans les assets de la release GitHub.")
        except Exception as e:
            print(f"Erreur critique durant la mise à jour : {e}")
        sys.exit(0)
...
def verifier_et_installer_maj_avec_ui(script_dir):
    try:
        # On utilise l'URL web publique des releases qui gère la redirection vers la dernière version sans quota d'API
        release_url = "https://github.com/Ludo-PersoDev/EasyFacture/releases/latest"
