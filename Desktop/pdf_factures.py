...
def obtenir_logo_local():
    """
    Récupère le logo directement depuis le dossier local 'assets/logo.png'.
    """
...
def uploader_et_lier_pdf(local_pdf_path, facture_id):
    try:
        supabase = database.get_db()
        
        # Récupération sécurisée de l'ID de l'utilisateur connecté
        ...
        if not os.path.exists(local_pdf_path):
            print(f"[PDF CLOUD] ❌ Erreur : Le fichier local n'existe pas à l'adresse {local_pdf_path}")
            return None
        ...
        supabase.table("factures").update({
            "pdf_url": public_url
        }).eq("id", facture_id).execute()
        
        print(f"[PDF CLOUD] ✓ Facture synchronisée : {public_url}")
        return public_url

    except Exception as e:
        print(f"[PDF CLOUD] ❌ Erreur détaillée Uploader Facture : {e}")
        return None
...
def generer_pdf_facture_condensee(facture_id, output_path=None):
    ...
    # Intégration du logo depuis le dossier local assets
    logo_path = obtenir_logo_local()
    if logo_path:
        try:
    ...
    """
    if facture_dict.get("contact"): client_text += f"À l'attention de : {facture_dict['contact']}<br/>"
    if facture_dict.get("client_adresse"): client_text += f"{facture_dict['client_adresse']}<br/>"
    if facture_dict.get("client_cp") or facture_dict.get("client_ville"): client_text += f"{facture_dict.get('client_cp', '')} {facture_dict.get('client_ville', '')}<br/>"
    if facture_dict.get("client_siret") and not facture_dict.get("est_particulier"): client_text += f"SIRET : {facture_dict['client_siret']}<br/>"
    ...
    prest_table = Table(table_data, colWidths=[7.5 * cm, 2.5 * cm, 2.8 * cm, 2.2 * cm, 3.0 * cm])
    t_style = [
    ...
    uploader_et_lier_pdf(output_path, facture_id)
...
def generer_pdf_recap_facture(facture_id, output_path=None, intitule="Intervention(s)"):
    ...
    style_title = ParagraphStyle("TitleStyle", parent=styles["Normal"], fontSize=16, leading=20, textColor=COLOR_PRIMARY, fontName="Helvetica-Bold")
    style_sub = ParagraphStyle("SubStyle", parent=styles["Normal"], fontSize=10, leading=14, textColor=COLOR_TEXT)
    style_th = ParagraphStyle("THStyle", parent=styles["Normal"], fontSize=9, leading=12, textColor=colors.white, fontName="Helvetica-Bold", alignment=1)
    style_cell = ParagraphStyle("Cell", parent=styles["Normal"], fontSize=8, leading=11, textColor=COLOR_TEXT)
    style_cell_center = ParagraphStyle("CellC", parent=styles["Normal"], fontSize=8, leading=11, textColor=COLOR_TEXT, alignment=1)
