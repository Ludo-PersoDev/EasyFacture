...
def initialiser_bdd():
    ...
    if cursor.fetchone()[0] == 0:
        cursor.execute("INSERT INTO parametres (id, tva_exoneree) VALUES (1, 1)")
...
def generer_numero_document(type_doc: str) -> str:
    ...
    try:
        cursor.execute(f"SELECT {colonne} FROM {table} WHERE {colonne} LIKE ? ORDER BY id DESC LIMIT 1", (f"{prefixe}%",))
        row = cursor.fetchone()
        if row and row[0]:
...
if __name__ == "__main__":
    initialiser_bdd()
    print("✓ Base de données mise à jour avec le champ 'modele_facture' avec succès !")