...
DB_PATH = os.path.join(BASE_DIR, "FactureX.db")
...
def initialiser_bdd():
    ...
    # --- 7. TABLE DE TRAÇABILITÉ DES RESTAURATION ---
    cursor.execute("""
    ...
    if "backup_folder_path" not in cols_param:
        cursor.execute("ALTER TABLE parametres ADD COLUMN backup_folder_path TEXT DEFAULT 'auto_backups'")

    cursor.execute("PRAGMA table_info(clients)")
    cols_client = [column[1] for column in cursor.fetchall()]
    if "modele_facture" not in cols_client:
        cursor.execute("ALTER TABLE clients ADD COLUMN modele_facture TEXT DEFAULT 'condense'")
    ...
    if "facture_id" not in cols_interv:
        cursor.execute("ALTER TABLE interventions ADD COLUMN facture_id INTEGER REFERENCES factures(id) ON DELETE SET NULL")

    cursor.execute("PRAGMA table_info(factures)")
    cols_fact = [column[1] for column in cursor.fetchall()]
    if "pdf_path" not in cols_fact:
        cursor.execute("ALTER TABLE factures ADD COLUMN pdf_path TEXT DEFAULT ''")
    if "date_envoi_mail" not in cols_fact:
        cursor.execute("ALTER TABLE factures ADD COLUMN date_envoi_mail TEXT DEFAULT ''")
    if "conditions_reglement" not in cols_fact:
        cursor.execute("ALTER TABLE factures ADD COLUMN conditions_reglement TEXT DEFAULT 'Paiement à 30 jours'")
    if "mode_reglement" not in cols_fact:
        cursor.execute("ALTER TABLE factures ADD COLUMN mode_reglement TEXT DEFAULT 'Virement bancaire'")
    ...
    chemin_logo_destination = os.path.join("assets", "logo.png")
    
    # On copie le logo par défaut s'il n'existe pas
    if not os.path.exists(chemin_logo_destination):
        if os.path.exists("logo_initial.png"):
            shutil.copy("logo_initial.png", chemin_logo_destination)
            print("✓ Logo par défaut initialisé dans assets/logo.png")
    
    # On s'assure que la BDD pointe bien vers ce chemin par défaut
    if os.path.exists(chemin_logo_destination):
        conn = get_conn()
        conn.execute("UPDATE parametres SET logo_path = ? WHERE id = 1 AND (logo_path IS NULL OR logo_path = '')", 
                     (chemin_logo_destination,))
        conn.commit()
        conn.close()
...
def generer_numero_document(type_doc: str) -> str:
    ...
    try:
        cursor.execute(
            f"SELECT {colonne} FROM {table} WHERE {colonne} LIKE ? ORDER BY id DESC LIMIT 1",
            (f"{prefixe}%",),
        )
        row = cursor.fetchone()
        if row and row[0]:
...
def get_backup_path():
    try:
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute("SELECT backup_folder_path FROM parametres WHERE id = 1")
        row = cursor.fetchone()
        conn.close()
        return row["backup_folder_path"] if row and row["backup_folder_path"] else "auto_backups"
    except Exception:
        return "auto_backups"
...
def save_backup_path(path_str):
    try:
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute("UPDATE parametres SET backup_folder_path = ? WHERE id = 1", (path_str,))
        conn.commit()
        conn.close()
...
def verifier_progression_onboarding():
    """
    Détermine le niveau d'avancement de la configuration de l'application :
    - Niveau 1 : Entreprise non configurée (SIRET ou Nom vide) -> Tout bloqué sauf Paramètres.
    - Niveau 2 : Entreprise OK mais Catalogue vide -> Paramètres, Catalogue et Maintenance débloqués.
    - Niveau 3 : Entreprise + Catalogue OK mais Clients vide -> Ajout de Clients débloqué.
    - Niveau 4 : Tout est OK -> Accès total à l'application.
    """
    params = recuperer_parametres()
    nom = params.get("nom_entreprise", "").strip()
    siret = params.get("siret", "").strip()
    
    # Niveau 1 : Entreprise non configurée
    ...
    nb_catalogue = conn.execute("SELECT COUNT(*) FROM prestations").fetchone()[0]
    if nb_catalogue == 0:
        conn.close()
        return 2
