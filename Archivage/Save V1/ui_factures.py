...
class FacturesTab:
    ...
    def setup_ui(self):
        ...
        # 2. Le Treeview (le même, mais on changera ses colonnes)
    ...
    def setup_ui(self):
        ...
        self.combo_client = ttk.Combobox(frame, values=[c[1] for c in database.recuperer_tous_les_clients()])
        self.combo_client.pack(side="left", padx=5)
        ctk.CTkButton(frame, text="Charger à Facturer", command=self.charger_a_facturer).pack(side="left", padx=5)

        # Tableau des prestations à facturer
        self.tree = ttk.Treeview(self.tab, columns=("Check", "ID", "Date", "Presta", "Qté", "Prix HT", "Total"), show='headings')
        
        # Configuration des entêtes
        self.tree.heading("Check", text="Sélection")
        self.tree.column("Check", width=70, anchor="center")
        
        self.tree.heading("ID", text="ID")
        self.tree.column("ID", width=0, stretch=False) # ID caché (utilisé pour la BDD)
        self.tree.heading("Date", text="Date")
        self.tree.column("Date", width=80)
        self.tree.heading("Presta", text="Prestation")
        ...
        # Lier le clic pour cocher/décocher
        self.tree.bind("<Button-1>", self.toggle_check)

        self.date_debut = DateEntry(frame, width=12, background='darkblue', foreground='white', borderwidth=2, date_pattern='dd/mm/yyyy')
        self.date_debut.pack(side="left", padx=5)

        self.date_fin = DateEntry(frame, width=12, background='darkblue', foreground='white', borderwidth=2, date_pattern='dd/mm/yyyy')
        self.date_fin.pack(side="left", padx=5)

        ctk.CTkButton(frame, text="Sélectionner entre dates", command=self.selectionner_par_dates).pack(side="left", padx=5)

        # Action finale
        ctk.CTkButton(self.tab, text="Facturer les sélectionnés", fg_color="green", command=self.valider_facturation).pack(pady=10)
    ...
    def charger_a_facturer(self):
        ...
        if c_id:
            ...
            for row in rows:
                # Convertir explicitement le Row en tuple ici
    ...
    def setup_columns_a_facturer(self):
        self.tree["columns"] = ("check", "id", "client", "prestation", "prix")
        self.tree.heading("Check", text="Sélection")
        self.tree.column("Check", width=70, anchor="center")
        self.tree.heading("ID", text="ID")
        self.tree.column("ID", width=0, stretch=False) # ID caché (utilisé pour la BDD)
